...
<div id="isian">
	<p><h1>Quiz Siapa Berani</h1></p>
 	<div align="justify" style="padding-right:20px; color:#000000;">
    <div align="left">Silahkan pilih salah satu jawaban yang benar. Selamat Mengerjakan...
  </div>
  <b>Nama  Anda : <?=$znama;?></b>
  <hr />
<div id="scroll">
<? 
   ...
   echo ("<ol>");
   $no=1;
   echo form_open('latihan_soal/proses');
   foreach($hasil->result() as $row)
   {   	  
   	  echo ("<li>$no. $row->pertanyaan </li>");
      ?>
   	  <ol><li>a. <input type='radio' name='<?=$no;?>' value='a'><?=$row->opt1 ;?></li>
   	  <li>b. <input type='radio' name='<?=$no;?>' value='b'><?=$row->opt2 ;?></li>
   	  <li>c. <input type='radio' name='<?=$no;?>' value='c'><?=$row->opt3 ;?></li>
   	  <li>d. <input type='radio' name='<?=$no;?>' value='d'><?=$row->opt4 ;?></li></ol>
   	  <? $arr[]=$no;
   	     $no++;  	   
    }
echo ("</div>");

...
class Calendar extends Controller {
  ...
  function dayevents ($day){
		...
		$data['main'] = 'dosen/calendar_day_events';
		$this->load->vars($data);
		$this->load->view('dosen/calendar_day_events_view', $data); 	
  }
  ...
  function create(){
	...
	if(isset($_POST['add']))
	{
		...
		$data['main'] = 'dosen/calendar_create';
		//$this->load->vars($data);
		$this->load->view('dosen/calendar_create_temp',$data); 
		/*
	 ...
	 function edit($id){
	  ...
	  if(isset($_POST['add']))
	  {
		  ...
		  if((isset($_POST['date']) && !empty($_POST['date'])) && (isset($_POST['eventTitle']) && !empty($_POST['eventTitle'])) && (isset($_POST['eventContent']) && !empty($_POST['eventContent'])))	
		  {
			  ...
			  redirect('dosen/calendar/index','refresh');
		  }
		  else 
		  {
			  //alert message for empty input
	...
	function update(){
		...
		redirect('dosen/calendar/update');
		
	}
	
	function delete($id=0){
		$this->MCalendar->deleteEvent($id);
		$this->session->set_flashdata('message', 'Event sukses terhapus.');
		redirect('dosen/calendar/index');
	}
	...
	function mycal($user_id=0){
		...
		// Set breadcrumb
		$this->bep_site->set_crumb($this->lang->line('userlib_calendar_personal','dosen/calendar/admin/mycal/$user_id'));	
		$data['title'] = "Manage Calendar";
